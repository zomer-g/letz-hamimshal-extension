// Build GeoJSON (WGS84) + CSV rows (geometry_wkt in ITM/EPSG:6991) from the raw
// features extracted by content/mot-inject.js.
//
...
// (see skill govscraper-spatial): CSV `geometry_wkt` is ITM 6991 un-prefixed and
// last; GeoJSON is WGS84 (CRS84, lon/lat) and carries both bboxes. CRS is
// auto-detected per feature so the output stays correct even if the backend
// ever returns 3857 or WGS84 instead.
...
export function buildLayerOutputs(layer) {
  ...
  for (const f of feats) {
    ...
    const wktItm = crs === 'itm' ? f.wkt : geomToWkt(itmGeom);

    gj.push({ type: 'Feature', geometry: wgsGeom, properties: { ...f.attrs } });
    rows.push({ ...f.attrs, geometry_wkt: wktItm });
